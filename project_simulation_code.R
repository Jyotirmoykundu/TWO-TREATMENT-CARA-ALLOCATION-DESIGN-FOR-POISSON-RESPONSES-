...
#------------function to define R_hat
  ...
  Shi_n.hat=(0.5)*(exp( 10*(-log(a)) + (9*(-log(a))^2)/2) )
  ...
  m=mean(propA)
  sd=sd(propA)
  crit_val_1=-8.4968     # ---> T0[M-(M*0.05)]=-8.4968(2,2)
  crit_val_2=-7.94489359 # --->T01[M-(M*0.05)]
  test=0
  for(t in 1:M)
  {
    if(T1[t]<crit_val_1)
...
#same test statistic but different functions

plot(x,p,type="l",col="blue",lwd=2,main="Comparison of power of the test for lambda A=2 and different values of lambda B",xlab="Lambda B",ylab="Power",ylim=c(0,max(max(P1),max(p))))
lines(x,P1,col="red",lwd=2)
legend("topleft",col=c("red","blue"),legend=c("e^u/(1+e^u)","a^-u"),lty=1:1,lwd=2:2,bty="n")
grid()

#same function and dist but different test statistics

plot(x,p,type="l",col="green",lwd=2,main="Comparison of power for two seperate test statistics with lambdaA=2 and for different values of lambda B[a^-u]",xlab="Lambda B",ylab="Power",ylim=c(0,max(max(p_1),max(p))))
lines(x,p_1,col="purple",lwd=2)
legend("topleft",col=c("green","purple"),legend=c("test_stat_1","test_stat_2"),lty=1:1,lwd=2:2,bty="n")

#consistency for test stat 1
...
s=seq(100,400,50)
plot(s,C,type="l",col="red",lwd=2,main="Checking consistency of the test based on two test statistics[for a^-u]",xlab="sample size",ylab="Power",ylim=c(min(C),max(C)))
