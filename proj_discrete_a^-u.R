#project_sem4
#for discrete distribution of covariates(u)
...
T2=NULL     #for test statistic 2
 for(z in 1:M)
 {
  ...
  for(i in 1:n)
  {
    if(i<=n/2)
    {
     X[i]=rpois(n=1,lambda=lambda_A*a^(-u.trial[i]))
     delta[i]=1
    }
    else
    {
     X[i]=rpois(n=1,lambda=lambda_B*a^(-u.trial[i]))
     delta[i]=0
    }
  }
  Lam_nu.A=0
  ...
  for(i in 1:n)
  {
    if(delta[i]==1)
    {
     ...
     Lam_de.A=Lam_de.A+a^(-u[i])
    }
    else
    {
     Lam_nu.B=Lam_nu.B+X[i]
     Lam_de.B=Lam_de.B+a^(-u[i])
    }
  }
...
#------------function to define R_hat
  ...
  for(i in (n+1):N)
  {
    ...
    if(r<=Ra.hat) 
    {
      Xa=rpois(1,lambda=lA.hat*a^(-u[i]))
      X=c(X,Xa)
      delta[i]=1
      Lam_nu.A=Lam_nu.A+X[i]
      Lam_de.A=Lam_de.A+a^(-u[i])
    ...
    {
      Xb=rpois(1,lambda=lB.hat*a^(-u[i]))
      X=c(X,Xb)
      delta[i]=0
      Lam_nu.B=Lam_nu.B+X[i]
      Lam_de.B=Lam_de.B+a^(-u[i])
  ...
  countA[z]=sum(delta)
  propA[z]=countA[z]/N
  lambda_n.hat=( lA.hat*sum(delta*a^-u) + lB.hat*sum((1-delta)*a^-u) )/sum(a^(-u))
  Shi_n.hat=(0.5)*1.114
  ...
  m=mean(propA)
  sd=sd(propA)
  crit_val_1=-9.94125678   # ---> T0[M-(M*0.05)]=-9.94125678(2,2)
  crit_val_2=-8.56129523   #----> T01[M-(M*0.05)]
  test=0
  for(t in 1:M)
  {
    if(T1[t]<crit_val_1)
...
P.1=c(p.1,p.2,p.3,p.4,p.5,p.6,p.7,p.8,p.9,p.10)
...
P.11=c(p.11,p.12,p.13,p.14,p.15,p.16,p.17,p.18,p.19,p.20)
x=seq(2.4,6,0.4)
#same function and dist but different test statistics
plot(x,P.1,type="l",col="orange",lwd=2,main="Comparison of power for two seperate test statistics with lambdaA=2 and for different values of lambda B[a^-u]",xlab="Lambda B",ylab="Power",ylim=c(0,max(max(p_1),max(p))))
lines(x,P.11,col="purple",lwd=2)
legend("topleft",col=c("orange","purple"),legend=c("test_stat_1","test_stat_2"),lty=1:1,lwd=2:2,bty="n")
#same test statistic but different func
plot(P1,type="l",col="blue",lwd=2,main="Power of the test with different lambdaB where lambdaA=2[a^-U ,U is discretely distributed]",ylab="Power")
lines(x,P1,col="blue",lwd=2)
legend("topleft",col=c("red","blue"),legend=c("u+1/u+2","a^-u"),lty=1:1,lwd=2:2,bty="n")
grid()
#consistency
...
s=seq(100,400,50)
plot(s,C,type="l",col="red",lwd=2,main="Checking consistency of the test based on two test statistics[for a^-u]",xlab="sample size",ylab="Power",ylim=c(min(C,C_1),max(C,C_1)))
